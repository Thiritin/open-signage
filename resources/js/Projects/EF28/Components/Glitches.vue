...
import spritePurpleSrc from "@/Projects/EF28/Assets/images/smoke_purple3.png";
spritePurple.src = spritePurpleSrc;

let spriteGreen = new Image();
import spriteGreenSrc from "@/Projects/EF28/Assets/images/smoke_green3.png";
spriteGreen.src = spriteGreenSrc;

let spriteYellow = new Image();
import spriteYellowSrc from "@/Projects/EF28/Assets/images/smoke_yellow3.png";
spriteYellow.src = spriteYellowSrc;

let spriteBlue = new Image();
import spriteBlueSrc from "@/Projects/EF28/Assets/images/smoke_blue3.png";
spriteBlue.src = spriteBlueSrc;

let spriteRed = new Image();
import spriteRedSrc from "@/Projects/EF28/Assets/images/smoke_red3.png";
...
function MagicMist(uid, canvas, options = {}) {
    ...
    this.particleCount =
        options.particleCount && !isNaN(options.particleCount)
            ? options.particleCount
            : generateRandom(0, 10);
    this.maxxVelocity =
        options.maxxVelocity && !isNaN(options.maxxVelocity)
            ? options.maxxVelocity
            : generateRandom(0, 1);
    this.maxyVelocity =
        options.maxyVelocity && !isNaN(options.maxyVelocity)
            ? options.maxyVelocity
            : generateRandom(0, 1);
    this.disturbance = 0;
    ...
    if (this.canvas.getContext) {
        ...
        try {
            let storedParticles = JSON.parse(
                localStorage.getItem("layer_" + this.idx + "_particles")
            );

            let velocity = {
                maxxVelocity: this.maxxVelocity,
                maxyVelocity: this.maxyVelocity,
            };
            ...
            localStorage.setItem(
                "layer_" + this.idx + "_particles",
                JSON.stringify(this.particles)
            );
        } catch (e) {
            console.error(e);
        ...
        this.intervals.push(
            setInterval(function () {
                that.disturbance =
                    0.05 *
                    (that.disturbance -
                        generateRandom(-that.maxxVelocity, that.maxyVelocity));
            }, 10000)
        );
    };
    ...
    function Particle(
        ...
        this.maxyVelocity =
            options.maxyVelocity && !isNaN(options.maxyVelocity)
                ...
                : generateRandom(0, 1);
        this.offset =
            options.offset && !isNaN(options.offset)
                ? options.offset
                : generateRandom(
                      -2 * (this.scale * this.image.width),
                      2 * (this.scale * this.image.width)
                  );
        this.frequency =
            options.frequency && !isNaN(options.frequency)
                ? options.frequency
                : generateRandom(
                      0,
                      this.bufferCanvas.width /
        ...
        this.angle =
            options.angle && !isNaN(options.angle)
                ? options.angle
                : generateRandom(0, 2 * Math.PI);
        this.opacity =
            options.opacity && !isNaN(options.opacity) ? options.opacity : 0;
        ...
        this.target = {
            ...
            x: generateRandom(
                -(this.scale * this.image.width) / 2,
                this.bufferCanvas.width + (this.scale * this.image.height) / 2
            ),
            y: generateRandom(
                -(this.scale * this.image.height) / 2,
                this.bufferCanvas.height + this.scale * this.image.height
            ),
        ...
        this.possition.x =
            options.possition &&
            options.possition.x &&
            !isNaN(options.possition.x)
                ? options.possition.x
                : generateRandom(0, this.bufferCanvas.width);
        this.possition.y =
            options.possition &&
            options.possition.y &&
            !isNaN(options.possition.y)
                ? options.possition.y
                : this.bufferCanvas.height +
                  (this.scale * this.image.height) / 2;
        this.xInitialVelocity =
            options.xInitialVelocity && !isNaN(options.xInitialVelocity)
                ? options.xInitialVelocity
                : generateRandom(0, this.maxxVelocity);
        this.yInitialVelocity =
            options.yInitialVelocity && !isNaN(options.yInitialVelocity)
                ? options.yInitialVelocity
                : generateRandom(this.maxyVelocity / 2, this.maxyVelocity);
        this.xCurrentVelocity =
            options.xCurrentVelocity && !isNaN(options.xCurrentVelocity)
                ? options.xCurrentVelocity
                : this.xInitialVelocity;
        this.yCurrentVelocity =
            options.yCurrentVelocity && !isNaN(options.yCurrentVelocity)
                ? options.yCurrentVelocity
                : this.yInitialVelocity;
        ...
        this.reset = function () {
            ...
            this.offset = generateRandom(
                -2 * (this.scale * this.image.width),
                2 * (this.scale * this.image.width)
            );

            this.frequency = generateRandom(
                0,
                this.bufferCanvas.width / (this.scale * this.image.width) / 2
            );

            this.target.x = generateRandom(
                -(this.scale * this.image.width) / 2,
                this.bufferCanvas.width + (this.scale * this.image.height) / 2
            );
            this.target.y = generateRandom(
                -(this.scale * this.image.height) / 2,
                this.bufferCanvas.height + this.scale * this.image.height
            );
            ...
            if (Math.random() > 0.5) {
                ...
                this.possition.y = generateRandom(
                    -(this.scale * this.image.height) / 2,
                    this.bufferCanvas.height + this.scale * this.image.height
                );
            } else {
                this.possition.x = generateRandom(
                    -(this.scale * this.image.width) / 2,
                    this.bufferCanvas.width +
                        (this.scale * this.image.height) / 2
                );
            ...
            this.xCurrentVelocity = this.xInitialVelocity = generateRandom(
                0,
                this.maxxVelocity
            );
            this.yCurrentVelocity = this.yInitialVelocity = generateRandom(
                this.maxyVelocity / 2,
                this.maxyVelocity
            );
        ...
        this.draw = function () {
            ...
            if (this.image.src) {
                drawImage(
                    ...
                    this.image,
                    this.possition.x - (this.scale * this.image.width) / 2,
                    this.possition.y - (this.scale * this.image.height) / 2,
                    this.angle,
                    this.scale
                );
        ...
        this.update = function () {
            ...
            this.angle +=
                (this.frequency * this.xCurrentVelocity) / (180 * Math.PI);
            ...
            if (this.isFadeIn) {
                ...
                this.yCurrentVelocity =
                    this.yInitialVelocity + this.disturbance;
                this.possition.y += this.yCurrentVelocity;

                this.xCurrentVelocity =
                    this.xInitialVelocity + this.disturbance;
                this.possition.x += this.xCurrentVelocity;
            }
...
onMounted(() => {
    ...
    magicMistLayerController
        .add(
            new MagicMist(0, mistLayer.value, {
                particleCount: 40,
                maxxVelocity: 0.5,
                maxyVelocity: 0.5,
                particleOptions: {
...
<template>
    <canvas
        ref="mistLayer"
        id="MagicMistLayer1"
        style="z-index: 4; visibility: hidden"
